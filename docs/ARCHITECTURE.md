...
| BlockNote | 0.46.2 | 리치 텍스트 에디터 |
| Yjs | 13.6.29 | CRDT 기반 실시간 동기화 |
...
| Drizzle ORM | 0.45.1 | 타입 안전 쿼리 빌더 |
| NextAuth | 5.0.0-beta.30 | JWT 세션 인증 |
| y-websocket | 3.0.0 | Yjs WebSocket 서버 |

### 상태 관리

| 기술 | 버전 | 역할 |
|------|------|------|
| TanStack React Query | 5.90.20 | 서버 상태 관리 (캐싱, 동기화) |
| Zustand | 4.4.6 | 클라이언트 UI 상태 (모달, 검색) |
...
│   ├── use-documents.ts             # React Query 문서 훅
│   ├── use-search.ts               # 검색 모달 상태 (Zustand)
│   ├── use-settings.ts             # 설정 모달 상태 (Zustand)
│   ├── use-cover-image.ts          # 커버 이미지 모달 상태
│   └── use-presence.ts             # Yjs 접속자 상태
│
├── lib/                              # 유틸리티 & 라이브러리
│   ├── db/
│   │   ├── index.ts                 # DB 연결 (pg Pool + Drizzle)
...
│ parentDocument UUID     FK(self) │ ──┐ CASCADE DELETE
...
- **CASCADE DELETE**: 부모 삭제 시 자식 문서 자동 삭제
...
- 모든 인증 엔드포인트는 `session.user.id`로 소유권 검증
- 문서 수정/삭제 시 소유자 확인 (`userId` 매칭)
- 공개 문서는 `isPublished: true && isArchived: false` 조건 충족 필요
- 파일 업로드는 UUID 접두사로 파일명 충돌 방지
...
**Queries:**
- `useSearchDocs(query)` - 문서 검색
- `useSidebar(parentId?)` - 사이드바 트리 데이터
- `useTrash()` - 휴지통 목록
...
타입 안전한 fetch 래퍼로 모든 API 호출을 중앙 관리합니다:
...
│                         │  ┌──────────────────┐  │
│ • UserItem              │  │ Title  Collab  ⋮ │  │
│ • Search (⌘+K)         │  └──────────────────┘  │
│ • Settings              │                        │
│ • New Page (+)          │  Cover Image           │
│                         │  ┌──────────────────┐  │
│ Document Tree:          │  │ 🖼️ Cover          │  │
│ ├── Page 1              │  └──────────────────┘  │
│ │   ├── Sub Page A      │                        │
│ │   └── Sub Page B      │  Toolbar               │
│ ├── Page 2 ◄ active     │  ┌──────────────────┐  │
...
# 인증
AUTH_SECRET=<random-secret>           # JWT 서명 키
AUTH_OKTA_ID=<okta-client-id>         # (선택) Okta OAuth
AUTH_OKTA_SECRET=<okta-client-secret> # (선택) Okta OAuth
AUTH_OKTA_ISSUER=<okta-issuer-url>    # (선택) Okta OAuth
...
| 패키지 | 용도 |
|--------|------|
| `@blocknote/core`, `react`, `shadcn` | 리치 텍스트 에디터 |
...
| `emoji-picker-react` | 이모지 선택기 |
| `lucide-react` | 아이콘 라이브러리 |
| `next-auth` | 인증 (JWT + OAuth) |
| `next-themes` | 테마 전환 |
| `react-dropzone` | 파일 드래그 앤 드롭 |
| `react-textarea-autosize` | 자동 높이 조절 텍스트영역 |
| `sonner` | 토스트 알림 |
| `y-websocket` + `yjs` | 실시간 CRDT 동기화 |
| `zod` | 런타임 스키마 검증 |
| `zustand` | 클라이언트 상태 관리 |
